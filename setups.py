...
background = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/background_100.png').convert('RGBA'))).float(),dim=2)<100).float()
    ...
    def update_envs(self,indices):
        ...
        for index in indices:
            ...
            if self.env_info[index]["type"] == "image":
                ...
                self.cond_mask[0,0,85:95,45:55]=1
                ...
                self.v_cond[index,0,int(object_y-object_h//2):int(object_y-object_h//2+object_h),int(object_x-object_w//2):int(object_x-object_w//2+object_w)] += (1+flow_v)*self.propeller
                self.v_cond[index,1,int(object_y-object_h//2):int(object_y-object_h//2+object_h),int(object_x-object_w//2):int(object_x-object_w//2+object_w)] += (1+flow_v)*self.propeller
